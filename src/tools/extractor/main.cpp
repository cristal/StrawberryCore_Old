...
int main(int argc, char * arg[])
{
    ...
    for (int i = 0; i < LANG_COUNT; i++)
    {
        ...
        if (FileExists(tmp1))
        {
            sLog.Write("Detected locale: %s", langs[i]);
            ...
            if(FirstLocale < 0)
            {
                ...
                sLog.Write("Detected base client build: %u", build);
                if(build != CLIENT_BUILD)
                {
                    sLog.Write("ERROR: your client is not up-to-date. Client build should be %u", CLIENT_BUILD);
                    return 0;
                }
                if(extractDBC)
    ...
    if(FirstLocale < 0)
    {
        sLog.Write("No locales detected");
        return 0;
    }
