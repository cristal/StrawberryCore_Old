...
int WMOGroup::ConvertToVMAPGroupWmo(FILE *output, WMORoot *rootWMO, bool pPreciseVectorData)
{
    ...
    if(pPreciseVectorData)
    {
        ...
        {
            sLog.Write("Error while writing file nbraches ID");
            exit(0);
        }
        int wsize = sizeof(uint32) + sizeof(unsigned short) * nIdexes;
        if(fwrite(&wsize, sizeof(int), 1, output) != 1)
        {
            sLog.Write("Error while writing file wsize");
            // no need to exit?
        }
        if(fwrite(&nIdexes, sizeof(uint32), 1, output) != 1)
        {
            sLog.Write("Error while writing file nIndexes");
            exit(0);
        }
        if(nIdexes >0)
        {
            ...
            {
                sLog.Write("Error while writing file indexarray");
                exit(0);
            }
        }
        ...
        {
            sLog.Write("Error while writing file nVertices");
            exit(0);
        }
        if(nVertices >0)
        {
            ...
            {
                sLog.Write("Error while writing file vectors");
                exit(0);
            }
        }
...
WMOInstance::WMOInstance(MPQFile &f,const char* WmoInstName, uint32 mapID, uint32 tileX, uint32 tileY, FILE *pDirfile)
{
    ...
    if(!input)
    {
        sLog.Write("WMOInstance::WMOInstance: couldn't open %s", tempname);
        return;
    }
