...
void ReadLiquidTypeTableDBC()
{
    ...
    sLog.Write("Done! (%u LiqTypes loaded)", (unsigned int)LiqType_count);
}
            ...
            if (fname->find(".wmo") != string::npos)
            {
                ...
                if ((n = fopen(szLocalFile, "rb"))== NULL)
                {
                    ...
                    if(p != 3)
                    {
                        ...
                        //sLog.Write("Extracting", *fname);
                        WMORoot * froot = new WMORoot(*fname);
                        if(!froot->open())
                        {
                            sLog.Write("Couldn't open RootWmo!!!");
                            delete froot;
                            continue;
                        }
                        ...
                        if(!output)
                        {
                            sLog.Write("couldn't open %s for writing!", szLocalFile);
                            success=false;
                        }
                        froot->ConvertToVMAPRootWmo(output);
                        int Wmo_nVertices = 0;
                        //sLog.Write("root has %d groups", froot->nGroups);
                        if(froot->nGroups !=0)
                        {
                            for (uint32 i=0; i<froot->nGroups; ++i)
                            {
                                ...
                                //sLog.Write("Trying to open groupfile %s",groupFileName);
                                string s = groupFileName;
                                WMOGroup * fgroup = new WMOGroup(s);
                                if(!fgroup->open())
                                {
                                    sLog.Write("Could not open all Group file for: %s",GetPlainName(fname->c_str()));
                                    file_ok=false;
                                    break;
                                }
...
void ParsMapFiles()
{
    ...
    for (unsigned int i=0; i<map_count; ++i)
    {
        ...
        {
            sLog.Write2("Processing Map %u[", map_ids[i].id);
            for (int x=0; x<64; ++x)
            {
                for (int y=0; y<64; ++y)
                {
                    ...
                    {
                        //sLog.Write2(id_filename,"%02u %02u %03u",x,y,map_ids[i].id);//!!!!!!!!!
...
bool fillArchiveNameVector(std::vector<std::string>& pArchiveNames)
{
    ...
    for (std::vector<std::string>::iterator i = searchLocales.begin(); i != searchLocales.end(); ++i)
    {
        ...
        if ((status.st_mode & S_IFDIR) == 0)
            continue;
        //sLog.Write("Found locale '%s'", i->c_str());
        locales.push_back(*i);
    }
    printf("\n");

    // open locale expansion and common files
    sLog.Write("Adding data files from locale directories.");
    ...
    sLog.Write("Scanning patch levels from data directory.");
    sprintf(path, "%spatch", input_path);
    if (!scan_patches(path, pArchiveNames))
        return(false);

    // now, scan for the patch levels in locale dirs
    sLog.Write("Scanning patch levels from locale directories.");
    bool foundOne = false;
    for (std::vector<std::string>::iterator i = locales.begin(); i != locales.end(); ++i)
    {
        sLog.Write("Locale: %s", i->c_str());
...
bool processArgv(int argc, char ** argv, const char *versionString)
{
    ...
    if(!result)
    {
        sLog.Write("Extract %s.",versionString);
        sLog.Write("%s [-?][-s][-l][-d <path>]", argv[0]);
        sLog.Write("   -s : (default) small size (data size optimization), ~500MB less vmap data.");
        sLog.Write("   -l : large size, ~500MB more vmap data. (might contain more details)");
        sLog.Write("   -d <path>: Path to the vector data source folder.");
        sLog.Write("   -? : This message.");
    }
    return result;
}
...
int main(int argc, char ** argv)
{
    ...
    else
    {
        ...
        {
            sLog.Write("Your output directory seems to be polluted, please use an empty directory!");
            sLog.Write("<press return to exit>");
            char garbage[2];
            scanf("%c", garbage);
            return 1;
        }
    ...
    sLog.Write("Extract %s. Beginning work ....",versionString);
    ...
    {
        sLog.Write("FATAL ERROR: None MPQ archive found by path '%s'. Use -d option with proper path.",input_path);
        return 1;
    }
    ...
    if(success)
    {
        ...
        {
            delete dbc;
            sLog.Write("FATAL ERROR: Map.dbc not found in data file.");
            return 1;
        }
        ...
        for(unsigned int x=0;x<map_count;++x)
        {
            ...
            sLog.Write("Map - %s",map_ids[x].name);
        }
    ...
    if(!success)
    {
        sLog.Write("ERROR: Extract %s. Work NOT complete.",versionString);
        sLog.Write("Precise vector data=%d.", preciseVectorData);
        sLog.Write("Press any key.");
        getchar();
    }

    sLog.Write("Extract %s. Work complete. No errors.",versionString);
    delete [] LiqType;
    return 0;
}