...
bool Model::open()
{
    ...
    {
        f.close();
        sLog.Write("Error loading model %s", filename.c_str());
        return false;
    }
    ...
    {
        //sLog.Write("not included %s", filename.c_str());
        f.close();
        return false;
    }
    return true;
...
bool Model::ConvertToVMAPModel(char * outfilename)
{
    ...
    if(!output)
    {
        sLog.Write("Can't create the output file '%s'",outfilename);
        return false;
    }
...
ModelInstance::ModelInstance(MPQFile &f,const char* ModelInstName, uint32 mapID, uint32 tileX, uint32 tileY, FILE *pDirfile)
{
    ...
    if(!input)
    {
        //sLog.Write("ModelInstance::ModelInstance couldn't open %s", tempname);
        return;
    }
