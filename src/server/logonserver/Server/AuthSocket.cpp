...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    sLog->outStaticDebug("[LogonChallenge] got header, body is %#04x bytes", remaining);
    ...
    _build = ch->build;
    _expversion = (LogonHelper::IsPostCataclysmAcceptedClientBuild(_build) ? POST_WOTLK_EXP_FLAG : NO_VALID_EXP_FLAG) |
                  (LogonHelper::IsPreCataclysmAcceptedClientBuild(_build) ? POST_BC_EXP_FLAG : NO_VALID_EXP_FLAG);
    ...
    if (result)
    {
        ...
        sLog->outBasic("[LogonChallenge] Banned ip %s tries to login!", ip_address.c_str());
    }
    else
    {
        ...
        if (res2)
        {
            ...
            {
                sLog->outStaticDebug("[LogonChallenge] Account '%s' is locked to IP - '%s'", _login.c_str(), fields[3].GetCString());
                sLog->outStaticDebug("[LogonChallenge] Player address is '%s'", ip_address.c_str());
                if (strcmp(fields[3].GetCString(), ip_address.c_str()))
                {
                    sLog->outStaticDebug("[LogonChallenge] Account IP differs");
                    pkt << (uint8) WOW_FAIL_SUSPENDED;
                    locked=true;
                }
                else
                    sLog->outStaticDebug("[LogonChallenge] Account IP matches");
            }
            else
                sLog->outStaticDebug("[LogonChallenge] Account '%s' is not locked to ip", _login.c_str());
            ...
            if (!locked)
            {
                ...
                if (banresult)
                {
                    ...
                    {
                        pkt << (uint8) WOW_FAIL_BANNED;
                        sLog->outBasic("[LogonChallenge] Banned account %s tries to login!", _login.c_str());
                    }
                    else
                    {
                        pkt << (uint8) WOW_FAIL_SUSPENDED;
                        sLog->outBasic("[LogonChallenge] Temporarily banned account %s tries to login!", _login.c_str());
                    }
                }
                else
                {
                    ...
                    sLog->outBasic("[LogonChallenge] account %s is using '%c%c%c%c' locale (%u)", _login.c_str (), ch->country[3], ch->country[2], ch->country[1], ch->country[0], GetLocaleByName(_localizationName));
                }
            }
        }
        else                                            //no account
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    else
    {
        ...
        sLog->outBasic("[LogonChallenge] account %s tried to login with wrong password!",_login.c_str ());
            ...
            if (PreparedQueryResult loginfail = LoginDatabase.Query(stmt))
            {
                ...
                if (failed_logins >= MaxWrongPassCount)
                {
                    ...
                    if (WrongPassBanType)
                    {
                        ...
                        sLog->outBasic("[LogonChallenge] account %s got banned for '%u' seconds because it failed to authenticate '%u' times",
                    ...
                    else
                    {
                        ...
                        sLog->outBasic("[LogonChallenge] IP %s got banned for '%u' seconds because account %s failed to authenticate '%u' times",
...
bool AuthSocket::_HandleReconnectChallenge()
{
    ...
    _build = ch->build;
    _expversion = (LogonHelper::IsPostCataclysmAcceptedClientBuild(_build) ? POST_BC_EXP_FLAG : NO_VALID_EXP_FLAG) | (LogonHelper::IsPreCataclysmAcceptedClientBuild(_build) ? PRE_BC_EXP_FLAG : NO_VALID_EXP_FLAG);
...
bool AuthSocket::_HandleRealmList()
{
    ...
    for (RealmList::RealmMap::const_iterator i = sRealmList->begin(); i != sRealmList->end(); ++i)
    {
        ...
        if ((_expversion & POST_BC_EXP_FLAG) || (_expversion & POST_WOTLK_EXP_FLAG))
        {
            ...
            if (!LogonHelper::IsPreCataclysmAcceptedClientBuild(i->second.gamebuild))
                continue;
