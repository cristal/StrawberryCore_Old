    ...
    switch(GetAuraType())
    {
            ...
            switch(GetSpellProto()->SpellFamilyName)
            {
                ...
                case SPELLFAMILY_MAGE:
                    ...
                    if (GetSpellProto()->SpellFamilyFlags[1] & 0x1 && GetSpellProto()->SpellFamilyFlags[2] & 0x8)
                    {
                        ...
                        DoneActualBenefit = float(caster->ApplyEffectModifiers(GetSpellProto(), m_effIndex, DoneActualBenefit));
                    }
                    // Fire Ward
        ...
        case SPELL_AURA_DUMMY:
            ...
            if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_SHAMAN && m_spellProto->SpellFamilyFlags[1] & 0x400)
            {
                ...
                // apply spellmods
                amount = int32(caster->ApplyEffectModifiers(GetSpellProto(), m_effIndex, (float)amount));
            }
            break;
        ...
        case SPELL_AURA_PERIODIC_DAMAGE:
            ...
            else if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_DRUID && m_spellProto->SpellFamilyFlags[0] & 0x00800000 && GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
            {
                ...
                amount += int32(CalculatePctF(caster->GetTotalAttackPowerValue(BASE_ATTACK), cp));
            }
            // Rend
            else if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_WARRIOR && GetSpellProto()->SpellFamilyFlags[0] & 0x20)
            {
                ...
                amount+=int32(caster->ApplyEffectModifiers(m_spellProto, m_effIndex, ((mwb_min + mwb_max) / 2 + ap * mws / 14000) * 0.2f));
    ...
    switch (GetSpellProto()->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            switch (GetId())
            {
                // Demonic Circle
                case 48018:
                    if (GameObject* obj = target->GetGameObject(GetId()))
                    {
            ...
            switch (GetId())
            {
                ...
                case 51690:
                {
                    ...
                    UnitList targets;
                    {
                        ...
                        CellPair p(Strawberry::ComputeCellPair(target->GetPositionX(), target->GetPositionY()));
                        Cell cell(p);
                        cell.data.Part.reserved = ALL_DISTRICT;

                        Strawberry::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck u_check(target, radius);
                        Strawberry::UnitListSearcher<Strawberry::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck> checker(target, targets, u_check);

                        TypeContainerVisitor<Strawberry::UnitListSearcher<Strawberry::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck>, GridTypeMapContainer > grid_object_checker(checker);
                        TypeContainerVisitor<Strawberry::UnitListSearcher<Strawberry::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck>, WorldTypeMapContainer > world_object_checker(checker);
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (triggeredSpellInfo)
    {
        if (Unit *triggerCaster = GetTriggeredSpellCaster(triggeredSpellInfo, caster, target))
        {
            ...
            sLog->outDebug("AuraEffect::HandlePeriodicTriggerSpellAuraTick: Spell %u Trigger %u", GetId(), triggeredSpellInfo->Id);
        }
    }
    else
    {
        Creature* c = target->ToCreature();
        const SpellEffectEntry *Effect = GetSpellProto()->GetSpellEffect((GetEffIndex()));
        if (!c || !caster || !sScriptMgr->OnDummyEffect(caster, GetId(), Effect, target->ToCreature()) ||
            !c->AI()->sOnDummyEffect(caster, GetId(), SpellEffIndex(GetEffIndex())))
            sLog->outDebug("AuraEffect::HandlePeriodicTriggerSpellAuraTick: Spell %u has non-existent spell %u in EffectTriggered[%d] and is therefor not triggered.", GetId(), triggerSpellId, GetEffIndex());
    }
}
...
void AuraEffect::HandlePeriodicTriggerSpellWithValueAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (SpellEntry const* triggeredSpellInfo = sSpellStore.LookupEntry(triggerSpellId))
    {
        if (Unit *triggerCaster = GetTriggeredSpellCaster(triggeredSpellInfo, caster, target))
        {
            ...
            sLog->outDebug("AuraEffect::HandlePeriodicTriggerSpellWithValueAuraTick: Spell %u Trigger %u", GetId(), triggeredSpellInfo->Id);
        }
    }
    else
        sLog->outDebug("AuraEffect::HandlePeriodicTriggerSpellWithValueAuraTick: Spell %u has non-existent spell %u in EffectTriggered[%d] and is therefor not triggered.", GetId(), triggerSpellId, GetEffIndex());
}
        ...
        switch (GetId())
        {
            ...
            case 38772: // Grievous Wound
            {
                uint32 percent =
                    ...
                    caster->CalculateSpellDamage(target, GetSpellProto(),GetEffIndex()+1) : 100;
...
{
    if (!caster || !caster->isAlive() || !target->isAlive() || !caster->GetHealth())
        return;
    ...
    sLog->outDebug("PeriodicTick: donator %u target %u damage %u.", caster->GetEntry(), target->GetEntry(), damage);
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_OBS_MOD_HEALTH)
    {
        ...
        case 16488: // Blood Craze Rank 1
            dmg = uint32(caster->GetMaxHealth() / 100 * 0.2);
            damage = dmg;
            break;
        case 16490: // Blod Craze Rank 2
            dmg = uint32(caster->GetMaxHealth() / 100 * 0.4);
            damage = dmg;
            break;
        case 16491: // Blod Craze Rank 3
            dmg = uint32(caster->GetMaxHealth() / 100 * 0.6);
            damage = dmg;
            break;
        default:
            break;
...
void AuraEffect::HandlePeriodicManaLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (caster->GetMaxPower(powerType) > 0)
        gainMultiplier = SpellMgr::CalculateSpellEffectValueMultiplier(GetSpellProto(), GetEffIndex(), caster);
...
void AuraEffect::HandlePhase(AuraApplication const *aurApp, uint8 mode, bool apply) const
{
    ...
    if (Player* player = target->ToPlayer())
    {
        ...
        player->GetSession()->SendSetPhaseShift(newPhase, NULL, true);
    }
    else
    {
        if (!newPhase)
...
void AuraEffect::HandleFeignDeath(AuraApplication const *aurApp, uint8 mode, bool apply) const
{
    ...
    {
        UnitList targets;
        Strawberry::AnyUnfriendlyUnitInObjectRangeCheck u_check(target, target, target->GetMap()->GetVisibilityRange());
        Strawberry::UnitListSearcher<Strawberry::AnyUnfriendlyUnitInObjectRangeCheck> searcher(target, targets, u_check);
        ...
        && (GetSpellProto()->AuraInterruptFlags & AURA_INTERRUPT_FLAG_IMMUNE_OR_LOST_SELECTION))
    {
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            if (target->ToPlayer()->InBattleground())
            {
                ...
                sOutdoorPvPMgr->HandleDropFlag((Player*)target, GetId());
        }
    }
    // stop handling the effect if it was removed by linked event
    if (apply && aurApp->GetRemoveMode())
...
void AuraEffect::HandleModTotalPercentStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if ((GetMiscValueB() & (1<<STAT_STAMINA)) && (maxHPValue > 0) && (m_spellProto->Attributes & SPELL_ATTR0_ABILITY))
    {
...
void AuraEffect::HandleProcTriggerSpellAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    if (SpellEntry const* triggeredSpellInfo = sSpellStore.LookupEntry(triggerSpellId))
    {
        sLog->outDebug("AuraEffect::HandleProcTriggerSpellAuraProc: Triggering spell %u from aura %u proc", triggeredSpellInfo->Id, GetId());
        triggerCaster->CastSpell(triggerTarget, triggeredSpellInfo, true, NULL, this);
    }
    else
        sLog->outDebug("AuraEffect::HandleProcTriggerSpellAuraProc: Could not trigger spell %u from aura %u proc, because the spell does not have an entry in Spell.dbc.", triggerSpellId, GetId());
}
...
void AuraEffect::HandleProcTriggerSpellWithValueAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    {
        int32 basepoints0 = GetAmount();
        sLog->outDebug("AuraEffect::HandleProcTriggerSpellWithValueAuraProc: Triggering spell %u with value %d from aura %u proc", triggeredSpellInfo->Id, basepoints0, GetId());
        triggerCaster->CastCustomSpell(triggerTarget, triggerSpellId, &basepoints0, NULL, NULL, true, NULL, this);
    }
    else
        sLog->outDebug("AuraEffect::HandleProcTriggerSpellWithValueAuraProc: Could not trigger spell %u from aura %u proc, because the spell does not have an entry in Spell.dbc.", triggerSpellId, GetId());
}
...
void AuraEffect::HandleProcTriggerDamageAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    sLog->outDebug("AuraEffect::HandleProcTriggerDamageAuraProc: Triggering %u spell damage from aura %u proc", damage, GetId());
...
void AuraEffect::HandleRaidProcFromChargeAuraProc(AuraApplication* aurApp, ProcEventInfo& /*eventInfo*/)
{
    ...
    switch (GetId())
    {
        ...
        case 43593:            // Cold Stare
            ...
            break;
        default:
            sLog->outDebug("AuraEffect::HandleRaidProcFromChargeAuraProc: received not handled spell: %u", GetId());
            return;
    }
    ...
    sLog->outDebug("AuraEffect::HandleRaidProcFromChargeAuraProc: Triggering spell %u from aura %u proc", triggerSpellId, GetId());
...
void AuraEffect::HandleRaidProcFromChargeWithValueAuraProc(AuraApplication* aurApp, ProcEventInfo& /*eventInfo*/)
{
    ...
    {
        sLog->outDebug("AuraEffect::HandleRaidProcFromChargeWithValueAuraProc: received not handled spell: %u", GetId());
        return;
    }
