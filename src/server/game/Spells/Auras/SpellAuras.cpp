...
void AuraApplication::_HandleEffect(uint8 effIndex, bool apply)
{
    ...
    sLog->outDebug("AuraApplication::_HandleEffect: %u, apply: %u: amount: %i", aurEff->GetAuraType(), apply, aurEff->GetAmount());
...
void AuraApplication::BuildUpdatePacket(ByteBuffer& data, bool remove) const
{
    ...
    data << uint8(aura->GetSpellProto()->GetStackAmount() ? aura->GetStackAmount() : aura->GetCharges());
...
uint8 Aura::BuildEffectMaskForOwner(SpellEntry const* spellProto, uint8 avalibleEffectMask, WorldObject* owner)
{
    ...
    switch(owner->GetTypeId())
    {
        ...
        case TYPEID_PLAYER:
            for (uint8 i = 0; i< MAX_SPELL_EFFECTS; ++i)
            {
                if (IsUnitOwnedAuraEffect(spellProto->GetSpellEffectIdByIndex(i)))
                    effMask |= 1 << i;
            }
            break;
        case TYPEID_DYNAMICOBJECT:
            for (uint8 i = 0; i< MAX_SPELL_EFFECTS; ++i)
            {
                if (spellProto->GetSpellEffectIdByIndex(i) == SPELL_EFFECT_PERSISTENT_AREA_AURA)
                    effMask |= 1 << i;
            }
            break;
        default:
...
void Aura::Update(uint32 diff, Unit* caster)
{
    if (m_duration > 0)
    {
        ...
        if (m_timeCla)
        {
            ...
            {
                if (int32 manaPerSecond = m_spellProto->GetManaPerSecond())
                {
                    m_timeCla += 1000 - diff;
...
void Aura::RefreshTimers()
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (HasEffect(i))
            GetEffect(i)->CalculatePeriodic(caster, true);
}
...
{
    uint8 maxProcCharges = m_spellProto->GetProcCharges();
...
bool Aura::ModStackAmount(int32 num, AuraRemoveMode removeMode)
{
    ...
    if ((num > 0) && (stackAmount > int32(m_spellProto->GetStackAmount())))
    {
...
void Spell_Stats(Player* player, uint32 row, int spellid, uint32 SpellNumber, bool perc, bool turn)
{
    ...
    if (turn == false)
    {
        if (SpellStats)
        {
            ...
            {
            case 2:
                percent = SpellStats->percent2;
                stattype = SpellStats->stattype2;
                value = SpellStats->value2;
                break;
            case 3:
                percent = SpellStats->percent3;
                stattype = SpellStats->stattype3;
                value = SpellStats->value3;
                break;
            case 4:
                percent = SpellStats->percent4;
                stattype = SpellStats->stattype4;
                value = SpellStats->value4;
                break;
            case 5:
                percent = SpellStats->percent5;
                stattype = SpellStats->stattype5;
                value = SpellStats->value5;
                break;
            }
    ...
    else
    {
        if (SpellStats)
        {
            ...
            if (percent != 0)
            {
                ...
                {
                case 2:
                    percent = player->GetPercValue2(SpellNumber);
                    break;
                case 3:
                    percent = player->GetPercValue3(SpellNumber);
                    break;
                case 4:
                    percent = player->GetPercValue4(SpellNumber);
                    break;
                case 5:
                    percent = player->GetPercValue5(SpellNumber);
                    break;
                }
            }
            player->_ApplyAuraBonuses(player, spellid, stattype, value, row, perc, true, false);
        }
    }
}
...
bool Aura::IsProcTriggeredOnEvent(AuraApplication* aurApp, ProcEventInfo& eventInfo) const
{
    ...
    {
        if (GetSpellProto()->GetEquippedItemClass() == ITEM_CLASS_WEAPON)
        {
            ...
            if (eventInfo.GetDamageInfo())
            {
                ...
                if (!item || item->IsBroken() || item->GetTemplate()->Class != ITEM_CLASS_WEAPON || !((1<<item->GetTemplate()->SubClass) & GetSpellProto()->GetEquippedItemSubClassMask()))
                    return false;
            }
        }
        else if (GetSpellProto()->GetEquippedItemClass() == ITEM_CLASS_ARMOR)
        {
            // Check if player is wearing shield
            Item *item = target->ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
            if (!item || item->IsBroken() || item->GetTemplate()->Class != ITEM_CLASS_ARMOR || !((1<<item->GetTemplate()->SubClass) & GetSpellProto()->GetEquippedItemSubClassMask()))
                return false;
        }
    }
...
void UnitAura::FillTargetMap(std::map<Unit *, uint8> & targets, Unit* caster)
{
    ...
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS ; ++effIndex)
    {
        ...
        else
        {
            ...
            {
                switch(GetSpellProto()->GetSpellEffectIdByIndex(effIndex))
                {
                    ...
                    {
                        targetList.push_back(GetUnitOwner());
                        Strawberry::AnyFriendlyUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius);
                        Strawberry::UnitListSearcher<Strawberry::AnyFriendlyUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
                        GetUnitOwner()->VisitNearbyObject(radius, searcher);
                        break;
                    }
                    case SPELL_EFFECT_APPLY_AREA_AURA_ENEMY:
                    {
                        Strawberry::AnyAoETargetUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius); // No GetCharmer in searcher
                        Strawberry::UnitListSearcher<Strawberry::AnyAoETargetUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
                        GetUnitOwner()->VisitNearbyObject(radius, searcher);
                        break;
                    }
...
void DynObjAura::FillTargetMap(std::map<Unit *, uint8> & targets, Unit* /*caster*/)
{
    ...
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
    {
        ...
        {
            Strawberry::AnyFriendlyUnitInObjectRangeCheck u_check(GetDynobjOwner(), dynObjOwnerCaster, radius);
            Strawberry::UnitListSearcher<Strawberry::AnyFriendlyUnitInObjectRangeCheck> searcher(GetDynobjOwner(), targetList, u_check);
            GetDynobjOwner()->VisitNearbyObject(radius, searcher);
        }
        else
        {
            Strawberry::AnyAoETargetUnitInObjectRangeCheck u_check(GetDynobjOwner(), dynObjOwnerCaster, radius);
            Strawberry::UnitListSearcher<Strawberry::AnyAoETargetUnitInObjectRangeCheck> searcher(GetDynobjOwner(), targetList, u_check);
            GetDynobjOwner()->VisitNearbyObject(radius, searcher);
        }
