...
void Spell::EffectEnvirinmentalDMG(SpellEffectEntry const* effect)
{
    uint32 absorb = 0;
    uint32 resist = 0;
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_DRUID:
            {
                ...
                if (m_caster->GetTypeId() == TYPEID_PLAYER && (m_spellInfo->SpellFamilyFlags[0] & 0x000800000) && m_spellInfo->SpellVisual[0] == 6587)
                {
                    ...
                    float multiple = ap / 410 + m_spellInfo->EffectDamageMultiplier[effect->Effect];
                ...
                if (m_caster->GetTypeId() == TYPEID_PLAYER && (m_spellInfo->SpellFamilyFlags[1] & 0x8))
                {
                    ...
                    if (uint32 combo = m_caster->ToPlayer()->GetComboPoints())
                    {
                        ...
                        if (AuraEffect const * aurEff = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE, 0x10000, 0, 0, m_caster->GetGUID()))
                        {
                            ...
                            damage *= doses;
                            damage += int32(((Player*)m_caster)->GetTotalAttackPowerValue(BASE_ATTACK) * 0.07f * combo);
                        }
...
void Spell::EffectDummy(SpellEffectEntry const* effect)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARRIOR:
            ...
            if (m_spellInfo->SpellFamilyFlags[EFFECT_0] & SPELLFAMILYFLAG_WARRIOR_EXECUTE)
            {
                ...
                bp = damage + int32(rageUsed * m_spellInfo->EffectDamageMultiplier[effect->Effect] + m_caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.2f);
                break;
            }
            // Concussion Blow
        ...
        // Divine Storm
        {
            if (m_spellInfo->SpellFamilyFlags[1] & SPELLFAMILYFLAG1_PALADIN_DIVINESTORM && effect->EffectIndex == 1)
            {
    ...
    sLog->outDebug("Spell ScriptStart spellid %u in EffectDummy(%u)", m_spellInfo->Id, effect);
    ...
    if (gameObjTarget)
        sScriptMgr->OnDummyEffect(m_caster, m_spellInfo->Id, effect, gameObjTarget);
    else if (unitTarget && unitTarget->GetTypeId() == TYPEID_UNIT)
        sScriptMgr->OnDummyEffect(m_caster, m_spellInfo->Id, effect, unitTarget->ToCreature());
    else if (itemTarget)
        sScriptMgr->OnDummyEffect(m_caster, m_spellInfo->Id, effect, itemTarget);
}
...
void Spell::EffectTriggerMissileSpell(SpellEffectEntry const* effect)
{
    ...
    if (!spellInfo)
    {
        sLog->outError("EffectTriggerMissileSpell of spell %u (eff: %u): triggering unknown spell id %u",
            m_spellInfo->Id,effect,triggered_spell_id);
        return;
    }
...
void Spell::EffectJumpDest(SpellEffectEntry const* effect)
{
    if (m_caster->isInFlight())
        return;
...
void Spell::EffectTeleportUnits(SpellEffectEntry const* /*effect*/)
{
    ...
    switch (m_spellInfo->Id)
    {
        ...
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(571, 5807.810059f, 587.976990f, 660.939026f, 1.663f);
            return;
        }
        case 3565: // Darnassus
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(1, 9660.809570f, 2513.639893f, 1331.652832f, 3.06f);
            return;
        }
        case 32271: // Exodar
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(530, -4029.929932f, -11572.200195f, -138.296005f, 2.43f);
            return;
        }
        case 3562: // Ironforge
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(0, -4613.620117f, -915.380005f, 501.062012f, 3.88f);
            return;
        }
        case 18960: // Moonglade
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(1, 7991.879883f, -2679.840088f, 512.099976f, 0.034183f);
            return;
        }
        case 3567: // Orgrimmar
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(1, 2032.800049f, -4336.700195f, 95.475403f, 1.5f);
            return;
        }
        case 33690: // Shattrath
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(1, -1823.709961f, 5416.970215f, -12.427400f, 2.790360f);
            return;
        }
        case 35715: // Shattrath
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(530, -1823.709961f, 5416.970215f, -12.427400f, 2.790360f);
            return;
        }
        case 32272: // Silvermoon
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(530, 9998.490234f, -7106.779785f, 47.705502f, 2.44f);
            return;
        }
        case 49358: // Stonard
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(0, -10442.0f, -3291.459961f, 20.179001f, 1.302920f);
            return;
        }
        case 3561: // Stormwind
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(0, -8209.799805f, 454.119995f, 117.430008f, 5.0f);
            return;
        }
        case 49359: // Theramore
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(1, -3730.719971f, -4422.209961f, 30.468203f, 0.810732f);
            return;
        }
        case 3566: // Thunder Bluff
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(1, -964.979980f, 283.433014f, 111.186996f, 3.02f);
            return;
        }
        case 88342: // Tol Barad
        {
            // NOTE: NEED COORDS
            //if (m_caster->GetTypeId() == TYPEID_PLAYER)
        ...
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(0, 1773.469971f, 61.120998f, -46.320702f, 0.54f);
            return;
        }
        case 52751: // Death Gate
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                m_caster->ToPlayer()->TeleportTo(0, 2353.530029f, -5665.819824f, 382.248993f, 0.596f);
            return;
        }
    }
...
{
    if (effect->EffectItemType == 6948 && unitTarget->ToPlayer()->GetItemByEntry(6948)) //dont create a new hearthstone on homebind if player already has
        return;
...
void Spell::EffectPersistentAA(SpellEffectEntry const* effect)
{
    if (!m_spellAura)
    {
        ...
        if (!dynObj->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_DYNAMICOBJECT), caster, m_spellInfo->Id, *m_targets.GetDst(), radius, false))
        {
            delete dynObj;
            return;
        }
...
void Spell::EffectEnergizePct(SpellEffectEntry const* effect)
{
    if (!unitTarget)
        return;
    if (!unitTarget->isAlive())
...
void Spell::EffectSummonType(SpellEffectEntry const* effect)
{
    ...
    switch (properties->Category)
    {
        ...
        case SUMMON_CATEGORY_PET:
            SummonGuardian(effect->EffectIndex, entry, properties);
            break;
        ...
        case SUMMON_CATEGORY_VEHICLE:
        {
            ...
            if (effect->EffectBasePoints)
            {
                SpellEntry const *spellProto = sSpellStore.LookupEntry(SpellMgr::CalculateSpellEffectAmount(m_spellInfo, effect->EffectIndex));
                if (spellProto)
                    m_caster->CastSpell(summon, spellProto, true);
            }

            m_caster->EnterVehicle(summon);
            break;
        }
    }
...
void Spell::EffectDispel(SpellEffectEntry const* effect)
{
    ...
    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        ...
        if ((1<<aura->GetSpellProto()->Dispel) & dispelMask)
        {
            ...
            {
                bool positive = aurApp->IsPositive() ? (!(aura->GetSpellProto()->AttributesEx & SPELL_ATTR1_CANT_BE_REFLECTED)) : false;
...
void Spell::EffectAddFarsight(SpellEffectEntry const* effect)
{
    ...
    if (!m_caster->IsInWorld())
        return;
    DynamicObject* dynObj = new DynamicObject;
    if (!dynObj->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_DYNAMICOBJECT), m_caster, m_spellInfo->Id, *m_targets.GetDst(), radius, true))
    {
        delete dynObj;
        return;
    }
...
void Spell::EffectAddHonor(SpellEffectEntry const* /*effect*/)
{
    ...
    if (damage <= 50)
    {
        uint32 honor_reward = Strawberry::Honor::hk_honor_at_level(unitTarget->getLevel(), float(damage));
...
void Spell::EffectEnchantItemTmp(SpellEffectEntry const* effect)
{
    ...
    if (!enchant_id)
    {
        sLog->outError("Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have 0 as enchanting id",m_spellInfo->Id,effect);
        return;
    }
    ...
    if (!pEnchant)
    {
        sLog->outError("Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) have not existed enchanting id %u ",m_spellInfo->Id,effect,enchant_id);
        return;
    }
...
void Spell::SpellDamageWeaponDmg(SpellEffectEntry const* effect)
{
    if (!unitTarget)
        return;
    if (!unitTarget->isAlive())
...
void Spell::EffectScriptEffect(SpellEffectEntry const* effect)
{
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
                ...
                case 58420:                                 // Portal to Stormwind
                {
                    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER || effect != 0)
                        return;
    ...
    // normal DB scripted effect
    sLog->outDebug("Spell ScriptStart spellid %u in EffectScriptEffect(%u)", m_spellInfo->Id, effect);
    m_caster->GetMap()->ScriptsStart(sSpellScripts, uint32(m_spellInfo->Id | (effect->Effect << 24)), m_caster, unitTarget);
}
...
void Spell::EffectDiscoverTaxi(SpellEffectEntry const* effect)
{
    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER)
        return;
...
void Spell::EffectGameObjectDamage(SpellEffectEntry const* /*effect*/)
{
...
void Spell::EffectGameObjectRepair(SpellEffectEntry const* /*effect*/)
{
    if (gameObjTarget && gameObjTarget->GetGoType() == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING)
        gameObjTarget->Rebuild();
}

void Spell::EffectGameObjectSetDestructionState(SpellEffectEntry const* effect)
{
    if (gameObjTarget && gameObjTarget->GetGoType() == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING)
    {
        Unit* caster = m_originalCaster;
...
void Spell::EffectPlayerNotification(SpellEffectEntry const* /*effect*/)
{
    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER)
        return;
...
void Spell::EffectRemoveAura(SpellEffectEntry const* effect)
{
    if (!unitTarget)
        return;
    // there may be need of specifying casterguid of removed auras
    unitTarget->RemoveAurasDueToSpell(effect->EffectTriggerSpell);
}
...
void Spell::EffectRechargeManaGem(SpellEffectEntry const* /*effect*/)
{
    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER)
        return;
