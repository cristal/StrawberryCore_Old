...
void SpellCastTargets::SetDst(SpellCastTargets const& spellTargets)
{
    ...
    m_dstPos.Relocate(spellTargets.GetDst());
    m_targetMask |= TARGET_FLAG_DEST_LOCATION;
}
...
void SpellCastTargets::ModDst(Position const& pos)
{
    ...
    {
        Position offset;
        GetDst()->GetPositionOffsetTo(pos, offset);
...
Spell::Spell(Unit* Caster, SpellEntry const *info, TriggerCastFlags triggerFlags, uint64 originalCasterGUID, bool skipCheck) :
    ...
    else if (m_damage > 0)
    {
        ...
        if (canEffectTrigger && missInfo != SPELL_MISS_REFLECT)
        {
            ...
            if (caster->GetTypeId() == TYPEID_PLAYER && (m_spellInfo->Attributes & SPELL_ATTR0_STOP_ATTACK_TARGET) == 0 &&
               (m_spellInfo->GetDmgClass() == SPELL_DAMAGE_CLASS_MELEE || m_spellInfo->GetDmgClass() == SPELL_DAMAGE_CLASS_RANGED))
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit *unit, const uint32 effectMask, bool scaleAura)
{
    ...
    if (aura_effmask)
    {
        ...
        if (scaleAura)
        {
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                basePoints[i] = aurSpellInfo->GetSpellEffectBasePoints(i);
        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                ...
                if (diminishMod == 0.0f)
                {
                    ...
                    bool found = false;
                    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                        if (effectMask & (1 << i) && m_spellInfo->GetSpellEffectIdByIndex(i) != SPELL_EFFECT_APPLY_AURA)
                            found = true;
                    if (!found)
                        return SPELL_MISS_IMMUNE;
                }
    ...
    switch(SpellTargetType[cur])
    {
        ...
        case TARGET_TYPE_DEST_CASTER: //4+8+2
        {
            ...
            if (cur == TARGET_MINION)
                ...
                dist = GetSpellRadiusForFriend(sSpellRadiusStore.LookupEntry(m_spellInfo->GetSpellEffect(i)->EffectRadiusIndex));
        ...
        case TARGET_TYPE_DEST_TARGET: //2+8+2
        {
            ...
            dist = (float)target->GetSpellRadiusForTarget(target, sSpellRadiusStore.LookupEntry(m_spellInfo->GetSpellEffect(i)->EffectRadiusIndex));
            if (dist < objSize)
                dist = objSize;
        ...
        case TARGET_TYPE_DEST_SPECIAL:
        {
            switch(cur)
            {
                case TARGET_DST_DB:
                    ...
                    {
                        //TODO: fix this check
                        if (m_spellInfo->GetSpellEffectIdByIndex(0) == SPELL_EFFECT_TELEPORT_UNITS || m_spellInfo->GetSpellEffectIdByIndex(1) == SPELL_EFFECT_TELEPORT_UNITS || m_spellInfo->GetSpellEffectIdByIndex(2) == SPELL_EFFECT_TELEPORT_UNITS)
    ...
    else if (pushType)
    {
        ...
        switch (targetType)
        {
                ...
                else
                {
                    ...
                    switch (m_spellInfo->Id)
                    {
                        ...
                        default:
                            ...
                            if (m_spellInfo->GetSpellEffectIdByIndex(i) == SPELL_EFFECT_TELEPORT_UNITS)
            ...
            case SPELL_TARGETS_GO:
            {
                ...
                {
                    if (m_spellInfo->GetSpellEffectIdByIndex(i) == SPELL_EFFECT_ACTIVATE_OBJECT)
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if (m_caster->IsControlledByPlayer() && !IsPassiveSpell(m_spellInfo->Id) && m_spellInfo->GetSpellLevel() && !IsChanneledSpell(m_spellInfo) && !(_triggeredCastFlags & TRIGGERED_IGNORE_AURA_SCALING))
    {
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (m_spellInfo->GetSpellEffectIdByIndex(i) == SPELL_EFFECT_APPLY_AURA)
            {
                ...
                {
                    m_auraScaleMask |= (1 << i);
                    if (m_spellValue->EffectBasePoints[i] != m_spellInfo->GetSpellEffectBasePoints(i))
                    {
                        m_auraScaleMask = 0;
                        break;
                    }
    ...
    sLog->outDebug("Spell::prepare: spell id %u source %u caster %d customCastFlags %u mask %u", m_spellInfo->Id, m_caster->GetEntry(), m_originalCaster ? m_originalCaster->GetEntry() : -1, _triggeredCastFlags, m_targets.GetTargetMask());

    //Containers for channeled spells have to be set
    //TODO:Apply this to all casted spells if needed
    // Why check duration? 29350: channelled triggers channelled
    if ((_triggeredCastFlags & TRIGGERED_CAST_DIRECTLY) && (!IsChanneledSpell(m_spellInfo) || !GetSpellMaxDuration(m_spellInfo)))
        cast(true);
    else
    {
        // stealth must be removed at cast starting (at show channel bar)
        // skip triggered spell (item equip spell casting and other not explicit character casts/item uses)
        if (!(_triggeredCastFlags & TRIGGERED_IGNORE_AURA_INTERRUPT_FLAGS) && isSpellBreakStealth(m_spellInfo))
        {
...
void Spell::handle_immediate()
{
    ...
    if (IsChanneledSpell(m_spellInfo))
    {
        ...
        if (duration)
        {
            ...
            m_caster->AddInterruptMask(m_spellInfo->GetChannelInterruptFlags());
...
void Spell::_handle_immediate_phase()
{
    ...
    for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j)
    {
        ...
        if (m_spellInfo->GetSpellEffectIdByIndex(j) == SPELL_EFFECT_SEND_EVENT && !HaveTargetsForEffect(j))
        {
    ...
    for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j)
    {
        ...
        if (EffectTargetType[m_spellInfo->GetSpellEffectIdByIndex(j)] == SPELL_REQUIRE_DEST)
        {
        ...
        else if (EffectTargetType[m_spellInfo->GetSpellEffectIdByIndex(j)] == SPELL_REQUIRE_NONE)
        {
            HandleEffects(m_originalCaster, NULL, NULL, j);
            m_effectMask |= (1<<j);
        }
...
void Spell::SendCastResult(Player* caster, SpellEntry const* spellInfo, uint8 cast_count, SpellCastResult result, SpellCustomErrors customError /*= SPELL_CUSTOM_ERROR_NONE*/)
{
    ...
    switch (result)
    {
        ...
        case SPELL_FAILED_TOO_MANY_OF_ITEM:
        {
             uint32 item = 0;
             for (int8 x=0;x < 3;x++)
                 if (spellInfo->GetEffectItemType(x))
                     item = spellInfo->GetEffectItemType(x);
...
void Spell::SendSpellGo()
{
    ...
    if ((IsTriggered() && !IsAutoRepeatRangedSpell(m_spellInfo)) || m_triggeredByAuraSpell)
    ...
    {
        castFlags |= CAST_FLAG_ADJUST_MISSILE;                   // same as in SMSG_SPELL_START
    ...
    */
    ...
    if (castFlags & CAST_FLAG_ADJUST_MISSILE)                   // unknown wotlk
    {
        data << float(0);
        data << uint32(0);
    }
    ...
    if (castFlags & CAST_FLAG_VISUAL_CHAIN)                   // unknown wotlk
    {
        data << uint32(0);
        data << uint32(0);
    }
...
void Spell::SendLogExecute()
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        data << uint32(m_spellInfo->GetSpellEffectIdByIndex(i));             // spell effect
...
void Spell::HandleEffects(Unit *pUnitTarget, Item *pItemTarget, GameObject *pGOTarget, uint32 i)
{
    ...
    uint8 eff = m_spellInfo->GetSpellEffectIdByIndex(i);
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (target)
    {
        ...
        if (!IsTriggered() && m_spellInfo->GetTargetAuraStateNot() && target->HasAuraState(AuraState(m_spellInfo->GetTargetAuraStateNot()), m_spellInfo, m_caster))
        ...
        if (non_caster_target)
        {
            ...
            if (!IsTriggered() && m_spellInfo->GetTargetAuraState() && !target->HasAuraState(AuraState(m_spellInfo->GetTargetAuraState()), m_spellInfo, m_caster))
                return SPELL_FAILED_TARGET_AURASTATE;
            ...
            if (!IsTriggered() && !m_caster->canSeeOrDetect(target))
                return SPELL_FAILED_BAD_TARGETS;
    ...
    if (!IsTriggered() && (m_spellInfo->Effect[EFFECT_0] == SPELL_EFFECT_DISPEL))
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        ...
        switch(m_spellInfo->GetSpellEffectIdByIndex(i))
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
                if (m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_WARRIOR)
                {
            ...
            {
                SummonPropertiesEntry const *SummonProperties = sSummonPropertiesStore.LookupEntry(m_spellInfo->GetEffectMiscValueB(i));
                if (!SummonProperties)
                    break;
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        switch(m_spellInfo->GetEffectApplyAuraNameByIndex(i))
        {
            ...
            case SPELL_AURA_MOUNTED:
            {
                ...
                if (m_caster->GetTypeId() == TYPEID_PLAYER && !AllowMount && !m_spellInfo->GetAreaGroupId())
                    ...
                    if (AreaTableEntry const* pArea = GetAreaEntryByAreaID(m_originalCaster->GetAreaId()))
                    {
                        ...
                        if (sWorld->getBoolConfig(CONFIG_OUTDOORPVP_WINTERGRASP_ENABLED))
                        {
                          if (OutdoorPvPWG *pvpWG = (OutdoorPvPWG*)sOutdoorPvPMgr->GetOutdoorPvPToZoneId(4197))
                            if (m_originalCaster->GetZoneId() == 4197 && pvpWG != 0)
    ...
    if (m_spellInfo->AttributesEx & SPELL_ATTR1_DISPEL_AURAS_ON_IMMUNITY)
    {
        for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (m_spellInfo->GetEffectApplyAuraNameByIndex(i) == SPELL_AURA_SCHOOL_IMMUNITY)
                ...
                dispel_immune |= GetDispelMask(DispelType(m_spellInfo->GetEffectMiscValue(i)));
        }
                    ...
                    if (AuraEffect * part = aura->GetEffect(i))
                    {
                        switch(part->GetAuraType())
                        {
                            case SPELL_AURA_MOD_STUN:
                                if (!usableInStun || !(GetAllSpellMechanicMask(aura->GetSpellProto()) & (1<<MECHANIC_STUN)))
...
SpellCastResult Spell::CheckItems()
{
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        switch (m_spellInfo->GetSpellEffectIdByIndex(i))
        {
            ...
            {
                if (!IsTriggered() && m_spellInfo->GetEffectItemType(i))
                {
                    ItemPosCountVec dest;
                    InventoryResult msg = p_caster->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, m_spellInfo->GetEffectItemType(i), 1);
                    if (msg != EQUIP_ERR_OK)
                    {
                        ItemTemplate const *pProto = sObjectMgr->GetItemTemplate(m_spellInfo->GetEffectItemType(i));
                        // TODO: Needs review
                        if (pProto && !(pProto->ItemLimitCategory))
                        {
                            p_caster->SendEquipError(msg, NULL, NULL, m_spellInfo->GetEffectItemType(i));
                            return SPELL_FAILED_DONT_REPORT;
                        }
                        else
                        {
                            ...
                            else if (!(p_caster->HasItemCount(m_spellInfo->GetEffectItemType(i), 1)))
                                return SPELL_FAILED_TOO_MANY_OF_ITEM;
                            else
            ...
            case SPELL_EFFECT_ENCHANT_ITEM:
                if (m_spellInfo->GetEffectItemType(i) && m_targets.GetItemTarget()
            ...
            {
                 uint32 item_id = m_spellInfo->GetEffectItemType(i);
        ...
        switch(lockInfo->Type[j])
        {
            ...
            case LOCK_KEY_SKILL:
            {
                ...
                if (skillId != SKILL_NONE)
                {
                    ...
                    if (m_spellInfo->GetEffectImplicitTargetAByIndex(effIndex) == TARGET_GAMEOBJECT_ITEM || m_spellInfo->GetEffectImplicitTargetBByIndex(effIndex) == TARGET_GAMEOBJECT_ITEM)
...
bool Spell::CanExecuteTriggersOnHit(uint8 effMask) const
{
    ...
    {
        if (m_spellInfo->GetSpellEffectIdByIndex(i) == SPELL_EFFECT_DUMMY)
            effMask &= ~(1<<i);
    }
    return effMask;
}
