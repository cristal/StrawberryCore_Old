...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<Strawberry::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<Strawberry::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
    CellPair standing_cell(Strawberry::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY()));
    ...
    for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
        ...
        for (uint32 x = cell_min.x_coord; x < cell_max.x_coord; ++x)
        {
            for (uint32 y = cell_min.y_coord; y < cell_max.y_coord; ++y)
            {
                ...
                Strawberry::DelayedUnitRelocation cell_relocation(cell, pair, *this, MAX_VISIBILITY_DISTANCE);
                TypeContainerVisitor<Strawberry::DelayedUnitRelocation, GridTypeMapContainer  > grid_object_relocation(cell_relocation);
                TypeContainerVisitor<Strawberry::DelayedUnitRelocation, WorldTypeMapContainer > world_object_relocation(cell_relocation);
...
void Map::MoveAllCreaturesInMoveList()
{
    while (!i_creaturesToMove.empty())
    {
        ...
        // calculate cells
        CellPair new_val = Strawberry::ComputeCellPair(cm.x, cm.y);
        Cell new_cell(new_val);
...
{
    Strawberry::VisibleNotifier notifier(*player);

    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();
    TypeContainerVisitor<Strawberry::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<Strawberry::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
