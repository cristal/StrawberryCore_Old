...
{
    CellPair p = Voragine::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY());
...
bool Map::Add(Player* player)
{
    ...
    CellPair p = Voragine::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
    if (p.x_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP || p.y_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP)
    {
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<Voragine::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<Voragine::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
    CellPair standing_cell(Voragine::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY()));
...
void Map::Update(const uint32 &t_diff)
{
    ...
    Voragine::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<Voragine::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<Voragine::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
        ...
        for (uint32 x = cell_min.x_coord; x < cell_max.x_coord; ++x)
        {
            for (uint32 y = cell_min.y_coord; y < cell_max.y_coord; ++y)
            {
                ...
                Voragine::DelayedUnitRelocation cell_relocation(cell, pair, *this, MAX_VISIBILITY_DISTANCE);
                TypeContainerVisitor<Voragine::DelayedUnitRelocation, GridTypeMapContainer  > grid_object_relocation(cell_relocation);
                TypeContainerVisitor<Voragine::DelayedUnitRelocation, WorldTypeMapContainer > world_object_relocation(cell_relocation);
...
Map::PlayerRelocation(Player* player, float x, float y, float z, float orientation)
{
    ...
    CellPair old_val = Voragine::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
    CellPair new_val = Voragine::ComputeCellPair(x, y);
...
void Map::MoveAllCreaturesInMoveList()
{
    while (!i_creaturesToMove.empty())
    {
        ...
        // calculate cells
        CellPair new_val = Voragine::ComputeCellPair(cm.x, cm.y);
        Cell new_cell(new_val);
...
bool Map::CheckGridIntegrity(Creature* c, bool moved) const
{
    ...
    CellPair xy_val = Voragine::ComputeCellPair(c->GetPositionX(), c->GetPositionY());
    Cell xy_cell(xy_val);
    if (xy_cell != cur_cell)
    {
...
{
    const char mapname = i_mapEntry->name[sWorld->GetDefaultDbcLocale()];
    std::stringstream s;
    s << mapname;
    std::string strmapname = s.str();
    //strmapname.c_str()
    return i_mapEntry ? strmapname.c_str() : "UNNAMEDMAP\x0";
}
...
void Map::UpdateObjectVisibility(WorldObject* obj, Cell cell, CellPair cellpair)
{
    ...
    cell.SetNoCreate();
    Voragine::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<Voragine::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
    cell.Visit(cellpair, player_notifier, *this, *obj, obj->GetVisibilityRange());
}

void Map::UpdateObjectsVisibilityFor(Player* player, Cell cell, CellPair cellpair)
{
    Voragine::VisibleNotifier notifier(*player);

    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();
    TypeContainerVisitor<Voragine::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<Voragine::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
bool Map::ActiveObjectsNearGrid(uint32 x, uint32 y) const
{
    ...
    for (MapRefManager::const_iterator iter = m_mapRefManager.begin(); iter != m_mapRefManager.end(); ++iter)
    {
        ...
        CellPair p = Voragine::ComputeCellPair(plr->GetPositionX(), plr->GetPositionY());
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        GridPair p = Voragine::ComputeGridPair(x, y);
        if (getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
            GridPair p2 = Voragine::ComputeGridPair(c->GetPositionX(), c->GetPositionY());
