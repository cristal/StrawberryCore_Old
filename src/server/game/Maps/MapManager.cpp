...
bool MapManager::CanPlayerEnter(uint32 mapid, Player* player, bool loginCheck)
{
    ...
    std::stringstream s;
    s << entry->name[player->GetSession()->GetSessionDbcLocale()];
    std::string newmapName = s.str();
    char const* mapName = newmapName.c_str();
    ...
    if (entry->IsRaid())
    {
        ...
        if ((!group || !group->isRaidGroup()) && !sWorld->getBoolConfig(CONFIG_INSTANCE_IGNORE_RAID))
        {
            ...
            player->GetSession()->SendAreaTriggerMessage(player->GetSession()->GetVoragineString(LANG_INSTANCE_RAID_GROUP_ONLY), mapName);
