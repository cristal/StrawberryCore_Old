...
void Guild::BankMoveItemData::LogAction(MoveItemData* pFrom) const
{
    ...
    if (!pFrom->IsBank() && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE) && m_pPlayer->GetSession()->GetSecurity() > SEC_PLAYER)       // TODO: move to scripts
        sLog->outDetail("GM %s (Account: %u) deposit item: %s (Entry: %d Count: %u) to guild bank (Guild ID: %u)",
...
void Guild::HandleRemoveMember(WorldSession* session, const std::string& name)
{
    ...
    else if (Member* pMember = GetMember(session, name))
    {
        ...
        {
            const uint64& guid = pMember->GetGUID();            // After call to DeleteMember pointer to member becomes invalid
...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint32 amount)
{
    ...
    {
        sLog->outDetail("GM %s (Account: %u) deposit money (Amount: %u) to pGuild bank (Guild ID %u)",
...
void Guild::_CreateDefaultGuildRanks(LocaleConstant loc)
{
    ...
    _CreateRank(sObjectMgr->GetStrawberryString(LANG_GUILD_MASTER,   loc), GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetStrawberryString(LANG_GUILD_OFFICER,  loc), GR_RIGHT_ALL);
    _CreateRank(sObjectMgr->GetStrawberryString(LANG_GUILD_VETERAN,  loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetStrawberryString(LANG_GUILD_MEMBER,   loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(sObjectMgr->GetStrawberryString(LANG_GUILD_INITIATE, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
}
