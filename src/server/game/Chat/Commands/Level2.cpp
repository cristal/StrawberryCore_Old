    ...
    if (show == "move" && target)
    {
        ...
        Map *map = chr->GetMap();
        {
            ...
            if (wpGuid != 0)
            {
                ...
                wpCreature->AddObjectToRemoveList();
                // re-create
                CreatureTemplate const *ci = sObjectMgr->GetCreatureTemplate(VISUAL_WAYPOINT);
                if (!ci)
                    return false;
...
bool ChatHandler::HandleWpShowCommand(const char* args)
{
    ...
    if (show == "on")
    {
        ...
        do
        {
            ...
            CreatureTemplate const *ci = sObjectMgr->GetCreatureTemplate(id);
            if (!ci)
                return false;
...
bool ChatHandler::HandleHonorAddCommand(const char* args)
{
    ...
    target->RewardHonor(NULL, 1, int32((amount * 100)/2.4));
    return true;
}
...
bool ChatHandler::HandleLookupTitleCommand(const char* args)
{
    ...
    for (uint32 areaflag = 0; areaflag < sAreaStore.GetNumRows (); ++areaflag)
    {
        ...
        if (areaEntry)
        {
            int loc = GetSessionDbcLocale ();
            std::stringstream s;
            s << areaEntry->area_name[loc];
            std::string namee = s.str();
...
bool ChatHandler::HandleTitlesAddCommand(const char* args)
{
    ...
    snprintf(titleNameStr, 80, (const char*)titleInfo->name[GetSessionDbcLocale()], targetName);
