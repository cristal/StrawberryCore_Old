...
{
    //MakeNotifyPacket(data, CHAT_JOINED_NOTICE); // Old
    *data << uint64(guid);
}
...
{
    //MakeNotifyPacket(data, CHAT_LEFT_NOTICE); // Old
    *data << uint64(guid);
}
...
{
    //MakeNotifyPacket(data, CHAT_PLAYER_NOT_FOUND_NOTICE); // old
    *data << name;
}
...
{
    //MakeNotifyPacket(data, CHAT_MODE_CHANGE_NOTICE); // Old
    *data << uint64(guid);
    *data << uint8(oldflags);
...
{
    //MakeNotifyPacket(data, CHAT_BANNED_NOTICE); // Old
}
...
{
    //MakeNotifyPacket(data, CHAT_THROTTLED_NOTICE); // Old
}
