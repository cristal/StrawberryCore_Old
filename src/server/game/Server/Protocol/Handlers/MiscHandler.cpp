...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket & /*recv_data*/)
{
    ...
    if (GetPlayer()->isInCombat())
        reason = 1;
    else if (GetPlayer()->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING_SLOW | MOVEMENTFLAG_FALLING))
...
void WorldSession::HandleAddFriendOpcode(WorldPacket & recv_data)
{
    ...
    std::string friendName = GetVoragineString(LANG_FRIEND_IGNORE_UNKNOWN);
...
void WorldSession::HandleAddIgnoreOpcode(WorldPacket & recv_data)
{
    ...
    std::string IgnoreName = GetVoragineString(LANG_FRIEND_IGNORE_UNKNOWN);
