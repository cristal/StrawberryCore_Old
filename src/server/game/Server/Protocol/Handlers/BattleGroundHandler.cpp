...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket & recv_data)
{
    ...
    recv_data.read_skip<uint32>();  // BG id? somehow it's always 0
    recv_data.read_skip<uint8>();   // unk
    recv_data.read_skip<uint8>();   // unk
    ...
    sLog->outDebug("WORLD: Recvd CMSG_BATTLEMASTER_JOIN Message");
    ...
    // As of 4.x you can not choose bg instance
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket & recv_data)
{
    ...
    sLog->outDebug("Battleground: arena team id %u, leader %s tries to queue for arena type %u", _player->GetArenaTeamId(arenaSlot), _player->GetName(), arenaType);
...
// Send that packet to player windows for invite player to join the war(NEED TEST)
void WorldSession::SendWGInvitePlayerToWar(uint32 BattleId,uint32 ZoneId,uint32 p_time)
{
    // Send packet 
    WorldPacket data(SMSG_BATTLEFIELD_MGR_ENTRY_INVITE, 12);
    data << uint32(BattleId); // (BattleId) the BattleId of WG
    data << uint32(ZoneId); // (ZoneId) the zone where the battle is (4197 for wg)
    data << uint32(time(NULL)+p_time); // (time) Time in second that the player have for accept

    _player->WGBattleBox = true;
    _player->WGBattleBoxTimer = 20000;
    
    // Sending the packet to player
    SendPacket(&data);
}

// Send that packet when payer click on cancel for war(NEED TEST)
void WorldSession::SendCancelWGInvitePlayerToWar(uint32 BattleId, uint32 ZoneId, uint32 p_time)
{
    WorldPacket data(-SMSG_BATTLEFIELD_MGR_ENTRY_INVITE, 5);

    data << uint32(ZoneId);                // map id
    data << uint64(0);                     // 3.3.5, unknown
    data << uint32(p_time);                // time to remove from queue, milliseconds
...
// Send that packet for inform player that player join queue(NEED TEST)
void WorldSession::SendWGQueueInviteResponce(uint32 BattleId,uint32 ZoneId)
{
...
// Send that packet after kicked a player(NEED TEST)
void WorldSession::SendWGKickedMessage(uint32 BattleId)
{
