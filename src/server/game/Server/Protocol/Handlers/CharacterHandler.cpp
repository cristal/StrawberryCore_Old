...
void WorldSession::HandleCharCreateOpcode(WorldPacket & recv_data)
{
    ...
    delete m_charCreateCallback.GetParam();  // Delete existing if any, to make the callback chain reset to stage 0
    m_charCreateCallback.SetParam(new CharacterCreateInfo(name, race_, class_, gender, skin, face, hairStyle, hairColor, facialHair, outfitId, recv_data));
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_GET_CHECK_NAME);
    stmt->setString(0, name);
    m_charCreateCallback.SetFutureResult(CharacterDatabase.AsyncQuery(stmt));
}
    ...
    switch (createInfo->Stage)
    {
        case 0:
        {
            if (result)
            {
                ...
                SendPacket(&data);
                delete createInfo;
                m_charCreateCallback.SetParam(NULL);
                m_charCreateCallback.FreeResult();
                return;
            }

            ASSERT(m_charCreateCallback.GetParam() == createInfo);

            PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_GET_SUM_REALMCHARS);
            stmt->setUInt32(0, GetAccountId());

            m_charCreateCallback.FreeResult();
            m_charCreateCallback.SetFutureResult(LoginDatabase.AsyncQuery(stmt));
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder * holder)
{
    ...
    SendAccountDataInitialized(PER_CHARACTER_CACHE_MASK);
...
void WorldSession::HandleTutorialFlag(WorldPacket & recv_data)
{
    ...
    if (index >= MAX_CHARACTER_TUTORIAL_VALUES)
        return;
...
{
    for (uint8 i = 0; i < MAX_CHARACTER_TUTORIAL_VALUES; ++i)
...
void WorldSession::HandleAlterAppearance(WorldPacket & recv_data)
{
    ...
    _player->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_GOLD_SPENT_AT_BARBER, uint32(Cost));
