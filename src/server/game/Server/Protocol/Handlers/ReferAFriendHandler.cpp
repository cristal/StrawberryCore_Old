...
{
    sLog->outDebug("WORLD: CMSG_GRANT_LEVEL");

    uint64 guid;
    recv_data.readPackGUID(guid);

    Player *target = sObjectAccessor->GetObjectInWorld(guid, _player);
...
{
    sLog->outDebug("WORLD: CMSG_ACCEPT_LEVEL_GRANT");

    uint64 guid;
    recv_data.readPackGUID(guid);

    Player *other = sObjectAccessor->GetObjectInWorld(guid, _player);
