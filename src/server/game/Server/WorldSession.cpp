...
m_sessionDbLocaleIndex(locale), m_latency(0), 
m_TutorialsChanged(false), recruiterId(recruiter),
...
void WorldSession::SendAccountDataInitialized(uint32 mask)
{
    WorldPacket data(SMSG_ACCOUNT_DATA_INITIALIZED, 4 + 1 + 4 + 8 * 4);
    data << uint32(0);      // Unix timestamp
    data << uint8(1);       // Always 0x01
...
{
    memset(m_Tutorials, 0, sizeof(uint32) * MAX_CHARACTER_TUTORIAL_VALUES);
...
void WorldSession::SaveTutorialsData(SQLTransaction &trans)
{
    ...
    stmt->setUInt32(MAX_CHARACTER_TUTORIAL_VALUES, GetAccountId());
    trans->Append(stmt);
...
void WorldSession::ProcessQueryCallbacks()
{
    ...
    if (m_charCreateCallback.IsReady())
    {
        PreparedQueryResult pResult;
        m_charCreateCallback.GetResult(pResult);
        HandleCharCreateCallback(pResult, m_charCreateCallback.GetParam());
...
void WorldPacket::compress(Opcodes enumVal)
{
    uint32 opcode = LookupOpcodeNumber(enumVal);
    if (opcode == 0)
        return;
