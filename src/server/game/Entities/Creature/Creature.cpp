...
CreatureBaseStats const* CreatureBaseStats::GetStats(uint32 entry, uint8 level)
{
    return sObjectMgr->GetCreatureStats(entry, level);
}
...
void Creature::RemoveCorpse(bool setSpawnTime)
{
    ...
    setDeathState(DEAD);
    RemoveAllAuras();    UpdateObjectVisibility();
    loot.clear();
...
bool Creature::UpdateEntry(uint32 Entry, uint32 team, const CreatureData *data)
{
    ...
    CreatureBaseStats const* stats = sObjectMgr->GetCreatureStats(GetEntry(), getLevel());
...
bool Creature::LoadCreaturesAddon(bool reload)
{
    ...
    if (!cainfo->auras.empty())
    {
        for (std::vector<uint32>::const_iterator itr = cainfo->auras.begin(); itr != cainfo->auras.end(); ++itr)
        {
            ...
            AddAura(*itr, this);
            sLog->outDebug("Spell: %u added to creature (GUID: %u Entry: %u)", *itr, GetGUIDLow(), GetEntry());
        }
    }
    return true;
}
