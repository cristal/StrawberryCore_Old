        ...
        for (GroupReference *itr = _group->GetFirstMember(); itr != NULL; itr = itr->next())
            if (Player* member = itr->getSource())
                if (member->isAlive() && member->IsAtGroupRewardDistance(_victim))
                {
                    ...
                    // 2.4. _maxNotGrayMember - maximum level of alive group member within reward distance,
                    //      for whom victim is not gray;
                    uint32 grayLevel = Strawberry::XP::GetGrayLevel(lvl);
...
inline void KillRewarder::_InitXP(Player* player)
{
    ...
    if (_isBattleGround || (!_isPvP && !_killer->GetVehicle()))
        _xp = Strawberry::XP::Gain(player, _victim);
}
    ...
    if (_maxLevel)
    {
        ...
        if (!_isBattleGround || _xp)
        {
            ...
            if (!_isBattleGround)
            {
                ...
                _groupRate = Strawberry::XP::xp_in_group_rate(_count, isRaid);
            }
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
    ...
    {
        GetSession()->SendAreaTriggerMessage(GetSession()->GetStrawberryString(LANG_LEVEL_MINREQUIRED), 68);
        return false;
    }
    else
...
void Player::Regenerate(Powers power)
{
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (isInCombat()) // Strawberry Updates Mana in intervals of 2s, which is correct
            ...
            if (CanJoinConstantChannelInZone(channel, current_zone))
            {
                if (!(channel->flags & CHANNEL_DBC_FLAG_GLOBAL))
                {
                    ...
                    if (channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY)
                        currentNameExt = sObjectMgr->GetStrawberryStringForDBCLocale(LANG_CHANNEL_CITY);
                    else
...
void Player::UpdateCombatSkills(Unit *pVictim, WeaponAttackType attType, bool defence)
{
    ...
    uint8 greylevel = Strawberry::XP::GetGrayLevel(plevel);
...
void Player::SendActionButtons(uint32 state) const
{
    ...
    /*
        state can be 0, 1, 2
        0 - Looks to be sent when initial action buttons get sent, however on Strawberry we use 1 since 0 had some difficulties
        1 - Used in any SMSG_ACTION_BUTTONS packet with button data on Strawberry. Only used after spec swaps on retail.
...
void Player::SendMessageToSetInRange(WorldPacket *data, float dist, bool self, bool own_team_only)
{
    ...
    Strawberry::MessageDistDeliverer notifier(this, data, dist, own_team_only);
    VisitNearbyWorldObject(dist, notifier);
}
...
bool Player::RewardHonor(Unit *uVictim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (honor_f <= 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            uint8 k_grey = Strawberry::XP::GetGrayLevel(k_level);
            ...
            honor_f = ceil(Strawberry::Honor::hk_honor_at_level_f(k_level) * (v_level - k_grey) / (k_level - k_grey));
...
// not used in Strawberry, but used in scripting code
...
//not used in Strawberry, function for external script library
...
void Player::SendQuestReward(Quest const *pQuest, uint32 XP, Object * questGiver)
{
    ...
    data << 10 * Strawberry::Honor::hk_honor_at_level(getLevel(), pQuest->GetRewHonorMultiplier());
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!isGameMaster() && ar)
    {
        ...
        {
            GetSession()->SendAreaTriggerMessage("%s", GetSession()->GetStrawberryString(LANG_INSTANCE_CLOSED));
            return false;
        }
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetStrawberryString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, sObjectMgr->GetItemTemplate(missingItem)->Name1.c_str());
                else if (LevelMin)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetStrawberryString(LANG_LEVEL_MINREQUIRED), LevelMin);
            }
            return false;
        }
    }
...
{
    uint8 v_level = pVictim->getLevel();
    uint8 k_grey  = Strawberry::XP::GetGrayLevel(getLevel());
