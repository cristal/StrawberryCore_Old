...
enum MovementFlags
{
    ...
    MOVEMENTFLAG_UNKNOWN               = 0x02000000,
    ...
    MOVEMENTFLAG_ONTRANSPORT           = 0x40000000,
...
private:
    ...
    SpellSchoolMask const m_schoolMask;
public:
    explicit HealInfo(Unit* _healer, Unit* _target, Unit* _attacker, Unit* _victim, DamageEffectType _damageType, 
        uint32 _heal, SpellEntry const* _spellInfo, SpellSchoolMask _schoolMask)
        : m_healer(_healer), m_target(_target), m_heal(_heal), m_spellInfo(_spellInfo), m_schoolMask(_schoolMask), 
        m_attacker(_attacker), m_victim(_victim), m_damageType(_damageType)
    {
        m_absorb = 0;
        m_resist = 0;
        m_block = 0;
...
class Unit : public WorldObject
{
    public:
        ...
        void CastSpell(Unit* Victim, SpellEntry const *spellInfo, TriggerCastFlags triggerFlags, Item *castItem= NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
        ...
        float GetMovementPitch() const { return m_movementInfo.pitch; }
