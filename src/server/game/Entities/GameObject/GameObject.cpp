...
void GameObject::CastSpell(Unit* target, uint32 spellId)
{
    ...
    {
        if (target)
            if (spellInfo->Id != 51852) // The Eye of Acherus
                target->CastSpell(target, spellInfo, true);
            else
                target->CastSpell(target, spellInfo, false); // IMPORTANT NOTE: Triggered must be false otherwise there will be some crash problems while clicking ESC!!
        return;
    }
