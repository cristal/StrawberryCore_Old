...
Vehicle::Vehicle(Unit* unit, VehicleEntry const* vehInfo, uint32 creatureEntry) : _me(unit), _vehicleInfo(vehInfo), _usableSeatNum(0), _creatureEntry(creatureEntry), m_bonusHP(0)
{
...
{
    sLog->outDebug("Vehicle::Uninstall Entry: %u, GuidLow: %u", _creatureEntry, _me->GetGUIDLow());
    RemoveAllPassengers();
...
{
    sLog->outDebug("Vehicle::Reset Entry: %u, GuidLow: %u", _creatureEntry, _me->GetGUIDLow());
...
{
    sLog->outDebug("Vehicle::RemoveAllPassengers. Entry: %u, GuidLow: %u", _creatureEntry, _me->GetGUIDLow());
...
{
    sLog->outDebug("Vehicle: Installing accessory entry %u on vehicle entry %u (seat:%i)", entry, GetCreatureEntry(), seatId);
...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
    ...
    sLog->outDebug("Unit %s enter vehicle entry %u id %u dbguid %u seat %d", unit->GetName(), _me->GetEntry(), _vehicleInfo->m_ID, _me->GetGUIDLow(), (int32)seat->first);
        ...
        && seat->first == 0 && seat->second.SeatInfo->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL)
    {
        ...
        if (VehicleScalingInfo const *scalingInfo = sObjectMgr->GetVehicleScalingInfo(_vehicleInfo->m_ID))
        {
            ...
            m_bonusHP = uint32(_me->GetMaxHealth() * (averageItemLevel * scalingInfo->scalingFactor));
            _me->SetMaxHealth(_me->GetMaxHealth() + m_bonusHP);
            _me->SetHealth(_me->GetHealth() + m_bonusHP);
        }
    }
...
void Vehicle::RemovePassenger(Unit* unit)
{
    ...
    sLog->outDebug("Unit %s exit vehicle entry %u id %u dbguid %u seat %d", unit->GetName(), _me->GetEntry(), _vehicleInfo->m_ID, _me->GetGUIDLow(), (int32)seat->first);
...
{
    sLog->outDebug("Vehicle::Dismiss Entry: %u, GuidLow %u", _creatureEntry, _me->GetGUIDLow());
    Uninstall();
