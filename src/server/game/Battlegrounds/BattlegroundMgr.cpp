                ...
                switch(bg->GetMapId())
                {
                    ...
                    case 628:                                   // IC
                        ...
                        *data << uint32(((BattlegroundICScore*)itr2->second)->BasesDefended);        // bases defended
                        break;
                    case 726:
                        *data << uint32(0x00000002);            // count of next fields                        *data << uint32(((BattlegroundTPScore*)itr2->second)->FlagCaptures);            // flag captures
                        *data << uint32(((BattlegroundTPScore*)itr2->second)->FlagReturns);             // flag returns
                        break;
                    case 761:
                        *data << uint32(0x00000002);            // count of next fields                        *data << uint32(((BattlegroundBGScore*)itr2->second)->BasesAssaulted);          // bases asssulted
...
void BattlegroundMgr::CreateInitialBattlegrounds()
{
    ...
    do
    {
        ...
        //sLog->outString("Creating battleground %s, %u-%u", bl->name[sWorld->GetDefaultDbcLocale()], MinLvl, MaxLvl);
        if (!CreateBattleground(bgTypeID, IsArena, MinPlayersPerTeam, MaxPlayersPerTeam, MinLvl, MaxLvl, (const char *)bl->name[sWorld->GetDefaultDbcLocale()], bl->mapid[0], AStartLoc[0], AStartLoc[1], AStartLoc[2], AStartLoc[3], HStartLoc[0], HStartLoc[1], HStartLoc[2], HStartLoc[3], scriptId))
            continue;
