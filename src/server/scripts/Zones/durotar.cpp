    ...
    public:
        ...
        struct npc_troll_volunteerAI : public ScriptedAI
        {
            npc_troll_volunteerAI(Creature* creature) : ScriptedAI(creature)
            {
                ...
                if (player)
                    me->GetMotionMaster()->MoveFollow(player, 5.0f, float(urand(120, 240)));
            }
...
#define SAY_DARKSPERAR_1      -1400001
#define SAY_SPITESCALE_1      -1400000
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        if (pPlayer)
        {
            ...
            if (pPlayer->GetQuestStatus(24786) == QUEST_STATUS_INCOMPLETE || pPlayer->GetQuestStatus(24754) == QUEST_STATUS_INCOMPLETE ||
                pPlayer->GetQuestStatus(24762) == QUEST_STATUS_INCOMPLETE || pPlayer->GetQuestStatus(24774) == QUEST_STATUS_INCOMPLETE ||
                pPlayer->GetQuestStatus(26276) == QUEST_STATUS_INCOMPLETE || pPlayer->GetQuestStatus(24642) == QUEST_STATUS_INCOMPLETE ||
                pPlayer->GetQuestStatus(24768) == QUEST_STATUS_INCOMPLETE || pPlayer->GetQuestStatus(24780) == QUEST_STATUS_INCOMPLETE)
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        switch(uiAction)
        {
            case GOSSIP_ACTION_INFO_DEF:
                if (pCreature->GetGUIDLow() == 239679)
                    pCreature->GetMotionMaster()->MovePath(9999992, false);
                else if (pCreature->GetGUIDLow() == 239282)
                    pCreature->GetMotionMaster()->MovePath(9999991, false);
                   
                npc_darkspear_jailorAI* pAI = CAST_AI(npc_darkspear_jailorAI,pCreature->AI());
                if (pAI)
                    pAI->StartMovingToCage();
                
                pPlayer->KilledMonsterCredit(39062, pCreature->GetGUID());
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_darkspear_jailorAI (pCreature);
    }

    struct npc_darkspear_jailorAI : public ScriptedAI
    {
        npc_darkspear_jailorAI(Creature* pCreature) : ScriptedAI(pCreature) 
        { }       
        ...
        void StartMovingToCage()
        {
            uiPhase = 1;
            uiTimer = 8000;
            DoScriptText(SAY_DARKSPERAR_1, me);
        }
            ...
            if (uiTimer <= uiDiff)
            {
                switch(uiPhase)
                {
                    ...
                    case 1:
                        ...
                        uiTimer = 8000;
                    case 2:
                        if (GameObject* pCage = me->FindNearestGameObject(201968, 5.0f))
                                pCage->SetGoState(GO_STATE_ACTIVE);

                        if (Creature* pSpitescale = me->FindNearestCreature(38142, 30.0f, true))
                        {
                            DoScriptText(SAY_SPITESCALE_1, pSpitescale);
                            if (me->GetGUIDLow() == 239679)                            
                                pSpitescale->GetMotionMaster()->MovePoint(0, -1144.48f, -5414.22f, 10.59f);
                            else if (me->GetGUIDLow() == 239282)
                                pSpitescale->GetMotionMaster()->MovePoint(0, -1149.88f, -5527.07f, 8.10f);

                            pSpitescale->setFaction(16);
                            pSpitescale->SetReactState(REACT_AGGRESSIVE);
                            pSpitescale->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                            pSpitescale->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_OOC_NOT_ATTACKABLE);
                        }
                        uiPhase = 3;
                        uiTimer = 2000;
                        break;
