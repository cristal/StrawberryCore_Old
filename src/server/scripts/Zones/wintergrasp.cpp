            ...
            if (OutdoorPvPWG *pvpWG = (OutdoorPvPWG*)sOutdoorPvPMgr->GetOutdoorPvPToZoneId(4197))
            {
                ...
                if (pvpWG->m_changeAlly != true) // If wg is switching (.wg switch)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (pvpWG->m_timer <= 3600000) // A hour before battle begin, the portal will disappear.
                            {
                                uiPortalTimer = 0;
                                uiPortalPhase = 0;
                                Check = true;
...
public:
    ...
    struct npc_wintergrasp_guardAI : public ScriptedAI
    {
        npc_wintergrasp_guardAI(Creature* pCreature) : ScriptedAI(pCreature) {}
        ...
        void JustDied(Unit *Killer)
        {
            if (Killer->ToPlayer())
                if (sWorld->getBoolConfig(CONFIG_OUTDOORPVP_WINTERGRASP_ENABLED))
                    if (OutdoorPvPWG *pvpWG = (OutdoorPvPWG*)sOutdoorPvPMgr->GetOutdoorPvPToZoneId(4197))
                    {
                        if (me->GetAreaId() == 4576 || me->GetAreaId() == 4577 || me->GetAreaId() == 4578 || me->GetAreaId() == 4575)
                            if ((pvpWG->getDefenderTeam() == TEAM_ALLIANCE && me->GetEntry() == CRE_GUARD_A) || (pvpWG->getDefenderTeam() == TEAM_HORDE && me->GetEntry() == CRE_GUARD_H))
                                me->SendZoneUnderAttackMessage(Killer->ToPlayer());

                        if (me->GetAreaId() == 4581 || me->GetAreaId() == 4582 || me->GetAreaId() == 4583)
                            if ((pvpWG->getAttackerTeam() == TEAM_ALLIANCE && me->GetEntry() == CRE_GUARD_A) || (pvpWG->getAttackerTeam() == TEAM_HORDE && me->GetEntry() == CRE_GUARD_H))
                                me->SendZoneUnderAttackMessage(Killer->ToPlayer());
                    }
        }
    };
