                ...
                switch (action)
                {
                    ...
                    case ACTION_ROTFACE_COMBAT:
                    {
                        ...
                        if (Creature* rotface = Unit::GetCreature(*me, instance->GetData64(DATA_ROTFACE)))
                        {
                            ...
                            if (list.size() > 4)
                            {
                                list.sort(Voragine::ObjectDistanceOrderPred(rotface));
                                do
                                {
                                    list.pop_back();
                                } while (list.size() > 4);
    ...
    public:
        ...
        class spell_putricide_eat_ooze_SpellScript : public SpellScript
        {
            ...
            void SelectTarget(std::list<Unit*>& targets)
            {
                ...
                targets.sort(Voragine::ObjectDistanceOrderPred(GetCaster()));
    ...
    public:
        ...
        class spell_putricide_mutated_transformation_SpellScript : public SpellScript
        {
            ...
            void HandleSummon(SpellEffIndex effIndex)
            {
                ...
                SummonPropertiesEntry const* properties = sSummonPropertiesStore.LookupEntry(uint32(GetSpellEffectEntry(GetSpellInfo()->Id, effIndex)->EffectMiscValueB));
