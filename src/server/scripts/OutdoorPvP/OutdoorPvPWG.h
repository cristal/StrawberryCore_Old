...
enum WGText
{
    WG_TEXT_DAMAGED                = 12061,
    WG_TEXT_DESTROYED            = 12060,
    WG_TEXT_ALLIANCE_CAPTURED    = 12059,
    WG_TEXT_ALLIANCE_ATTACKED    = 12058,
    WG_TEXT_HORDE_CAPTURED        = 12057,
    WG_TEXT_HORDE_ATTACKED        = 12056,
    WG_TEXT_FIRSTRANK            = 12055,
    WG_TEXT_SECONDRANK            = 12054,
    WG_TEXT_CAPTURED_FORTRESS    = 12053,
    WG_TEXT_BEGIN2                = 12052,
    WG_TEXT_BEGIN                = 12051,
    WG_TEXT_DEFENDED_FORTRESS    = 12050,
};
...
class OutdoorPvPWG : public OutdoorPvP
{
    protected:
        ...
        typedef std::vector<OutdoorPvPWG*> OutdoorPvPWGSet;
    ...
    public:
        ...
        bool m_changeAlly;
        bool m_changeHorde;
        OutdoorPvPWGSet  m_OutdoorPvPWGSet;
        ...
        uint32 GetMaximumPlayerLimit() { return m_maxplayer;}
        // Kick player from the wg, teleport him to kick point
        void KickPlayerFromWG(Player* player, bool leave);
        void QueueBoxSendedToPlayer(Player* player);
        void PlayerAnsweredForInviteToQueue(Player* plr, bool Answer);
        Group* GetFreeWGRaid(uint32 TeamId);
        bool AddOrSetPlayerToCorrectWGGroup(Player *plr);
        ...
        void SendRankWarning(Player* killer);
    ...
    protected:
        ...
        PlayerSet m_playersinqueue[2]; // Queue box sended to player
        PlayerSet m_playersinbattlebox[2]; // Battle box(Enter battle etc.)
        PlayerSet m_playersinqueuelist[2]; // Player in the queue
        GroupSet m_groups[2];
        ...
        uint32 m_maxplayer;
        uint32 m_TimeForResponseOfQueue;
