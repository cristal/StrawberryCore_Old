...
public:
    ...
    static bool HandleNpcSetOrAddEmoteCommand(ChatHandler* handler, const char* args)
    {
        ...
        {
            lowguid = pCreature->GetDBTableGUIDLow();
            if (WorldDatabase.PQuery("SELECT * FROM `creature_addon` WHERE `guid`=%u;", lowguid))
                WorldDatabase.PQuery("UPDATE `creature_addon` SET `emote`=%u WHERE `guid`=%u;", emote , lowguid); // Just change the emoteid
            else
            {
                handler->SendSysMessage("There isn't any data in the creature_addon for this creature but data will be added...");
                WorldDatabase.PExecute("INSERT INTO `creature_addon` (`guid`, `emote`) VALUES (%u, %u)", lowguid, emote);
                /*PreparedStatement* stmt = WorldDatabase.GetPreparedStatement(WORLD_ADD_CREATURE_ADDON); // Just add new Emote to creature_addon
                stmt->setUInt32(0, lowguid);
                stmt->setUInt32(1, emote);
                WorldDatabse.Execute(stmt);*/
            }
            pCreature->SetUInt32Value(UNIT_NPC_EMOTESTATE, emote);
            handler->SendSysMessage("Successful.");
        }
