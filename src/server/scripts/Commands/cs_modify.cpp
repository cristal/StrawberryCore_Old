...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand modifyCommandTable[] =
        {
            ...
            { "bwalk",          SEC_MODERATOR,      false, &HandleModifyBWalkCommand,         "", NULL },// IT IS NOT WORKING!!!
            ...
            { "honor",          SEC_MODERATOR,      false, &HandleModifyHonorCommand,         "", NULL },// IT IS NOT WORKING!!!
            { "rep",            SEC_GAMEMASTER,     false, &HandleModifyRepCommand,           "", NULL },// IT IS NOT WORKING AND THERE IS A CRASH PROBLEM
            { "arena",          SEC_MODERATOR,      false, &HandleModifyArenaCommand,         "", NULL },// IT IS NOT WORKING!!!
    ...
    static bool HandleModifyEnergyCommand(ChatHandler* handler, const char* args)
    {
        ...
        sLog->outDetail(handler->GetVoragineString(LANG_CURRENT_ENERGY), target->GetMaxPower(POWER_ENERGY));
    ...
    static bool HandleTaxiCheatCommand (ChatHandler* handler, const char* args)
    {
        ...
        // check online security
        else if (handler->HasLowerSecurity(chr, 0))
            return false;
    ...
    static bool HandleModifyBWalkCommand(ChatHandler* handler, const char* args) // IT IS NOT WORKING!!!
    {return false;
        /*if (!*args)
            return false;
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        ...
        uint32 moneyuser = (target->GetMoney() >> 32);

        if (addmoney < 0)
        {
            int32 newmoney = int32(moneyuser) + addmoney;

            sLog->outDetail(handler->GetVoragineString(LANG_CURRENT_MONEY), moneyuser, addmoney, newmoney);
            if (newmoney <= 0)
            {
        ...
        sLog->outDetail(handler->GetVoragineString(LANG_NEW_MONEY), moneyuser, addmoney, target->GetMoney());
    ...
    static bool HandleModifyHonorCommand (ChatHandler* handler, const char* args) // IT IS NOT WORKING!!!
    {
        if (!*args)
            return false;
    ...
    static bool HandleModifyRepCommand(ChatHandler* handler, const char* args) // IT IS NOT WORKING AND THERE IS A CRASH PROBLEM
    {
        if (!*args) return false;
        ...
        if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
        {
            ...
            {
                std::string rank = handler->GetVoragineString(ReputationRankStrIndex[r]);
                if (rank.empty())
                    continue;
    ...
    static bool HandleModifyArenaCommand(ChatHandler* handler, const char* args) // IT IS NOT WORKING!!!
    {return false;
        /*if (!*args)
            return false;
