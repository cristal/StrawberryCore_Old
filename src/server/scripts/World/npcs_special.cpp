...
enum eTrainingDummy
{
    ...
    NPC_CATACLYSM_TARGET_DUMMY_1               = 44548,
    NPC_CATACLYSM_TARGET_DUMMY_2               = 44794,
};
...
public:
    ...
    struct npc_training_dummyAI : Scripted_NoMovementAI
    {
        ...
        {
            uiResetTimer = 5000;
            if (uiEntry != NPC_CATACLYSM_TARGET_DUMMY_1 && uiEntry != NPC_CATACLYSM_TARGET_DUMMY_2)
                damage = 0;
        }

        void JustDied(Unit * /*killer*/)
        {
            if (uiEntry == NPC_CATACLYSM_TARGET_DUMMY_1 || NPC_CATACLYSM_TARGET_DUMMY_2)
                me->Respawn();
        }
        void EnterCombat(Unit * /*who*/)
        {
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        {
            pPlayer->SetEmoteState(0);
            Vehicle *vehicle = pCreature->GetVehicleKit();
            pPlayer->EnterVehicle(pCreature->ToUnit(), 0);
            if (pCreature->GetDistance2d(-5168.479980f, 734.567993f) <= 1)
                pCreature->GetMotionMaster()->MovePath(243306, false);
            if (pCreature->GetDistance2d(-5173.009766f, 734.880005f) <= 1)
                pCreature->GetMotionMaster()->MovePath(243416, false);
            if (pCreature->GetDistance2d(-5177.830078f, 735.213989f) <= 1)
                pCreature->GetMotionMaster()->MovePath(242886, false);
    ...
    {
        npc_sanitron_500AI(Creature* pCreature) : ScriptedAI(pCreature), vehicle(pCreature->GetVehicleKit()) 
        {
            assert(vehicle);
        }
            ...
            if (!vehicle->HasEmptySeat(0))
                if (uiTimer <= diff)
                {
                    ...
                    case 6:
                        ...
                        // Need real explosion effect of sanitron here
                        ++uiPhase;
                        uiTimer = 1000;
                        break;
                    case 7:
    ...
    CreatureAI *GetAI(Creature *pCreature) const
    {
        return new npc_sanitron_500AI(pCreature);
    }
};
...
#define GOSSIP_TORBEN "Send me to the surface, Torben."

enum eTorben
{
    QUEST_TO_THE_SURFACE       = 27674,
    TEXT_ID                    = 46293
};

class npc_torben_zapblast : public CreatureScript
{
public:
    npc_torben_zapblast() : CreatureScript("npc_torben_zapblast") {}
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        switch (uiAction)
        {
            case GOSSIP_ACTION_INFO_DEF+1:
                pPlayer->CLOSE_GOSSIP_MENU();
                pPlayer->TeleportTo(0, -5201.579590f, 478.079346f, 388.237976f, 5.358291f);
                break;
        }
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        if (status == QUEST_STATUS_INCOMPLETE || status == QUEST_STATUS_COMPLETE)
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_TORBEN, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);

        pPlayer->SEND_GOSSIP_MENU(TEXT_ID, pCreature->GetGUID());
        return true;
    }

    struct npc_torben_zapblastAI : public ScriptedAI
    {
        npc_torben_zapblastAI(Creature* pCreature) : ScriptedAI(pCreature) {}
    };

    CreatureAI *GetAI(Creature *pCreature) const
    {
        return new npc_torben_zapblastAI(pCreature);
    }
};
...
enum eDemonicTrainer
{
    SPELL_SUMMON_IMP   = 688,
    SPELL_SUMMON_VOID  = 697,
    SPELL_SUMMON_SUCC  = 712,
    SPELL_SUMMON_FELH  = 691,
    SPELL_SUMMON_FELG  = 30146,
    TEXT_NON_WARLOCKS  = 49294,
    TEXT_WARLOCKS      = 49295,
    NPC_DANE_WINSLOW   = 6373,
    NPC_KURGUL         = 5815,
    NPC_MATERO_ZESHUWAL= 23535,
};

#define GOSSIP_IMP  "Abandon Imp"
#define GOSSIP_VOID "Abandon Voidwalker"
#define GOSSIP_SUCC "Abandon Succubus"
#define GOSSIP_FELH "Abandon Felhunter"
#define GOSSIP_FELG "Abandon Felguard"

class npc_demonic_trainer : public CreatureScript
{
public:
    npc_demonic_trainer() : CreatureScript("npc_demonic_trainer") {}
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        {
            if (pPlayer->HasSpell(SPELL_SUMMON_IMP))
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_IMP, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
            if (pPlayer->HasSpell(SPELL_SUMMON_VOID))
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_VOID, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
            if (pPlayer->HasSpell(SPELL_SUMMON_SUCC))
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_SUCC, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+3);
            if (pPlayer->HasSpell(SPELL_SUMMON_FELH))
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_FELH, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+4);
            if (pPlayer->HasSpell(SPELL_SUMMON_FELG))
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_FELG, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+5);
            pPlayer->SEND_GOSSIP_MENU(TEXT_WARLOCKS, pCreature->GetGUID());
        }
        else
            pPlayer->SEND_GOSSIP_MENU(TEXT_NON_WARLOCKS, pCreature->GetGUID());

        return true;
    }

    struct npc_demonic_trainerAI : public ScriptedAI
    {
        npc_demonic_trainerAI(Creature* pCreature) : ScriptedAI(pCreature) 
        {
            uiEntry = pCreature->GetEntry();
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            if (uiEntry != NPC_DANE_WINSLOW && uiEntry != NPC_KURGUL && uiEntry != NPC_MATERO_ZESHUWAL)
                if (!me->isInCombat())
                    if (!me->GetPet())
                    {
                        if (uiTimer <= diff)
                        {
                            DoCast(SPELL_SUMMON_IMP);
                            uiTimer = 6000;
                        }
                        else uiTimer -= diff;
                    }
                    else
                    {
                        if (uiTimer == 6000)
                        {
                            if (Pet* pet = me->GetPet())
                                pet->GetMotionMaster()->MoveFollow(me, 2, 200);
    ...
    CreatureAI *GetAI(Creature *pCreature) const
    {
        return new npc_demonic_trainerAI(pCreature);
    }
};
...
void AddSC_npcs_special()
{
    ...
    new npc_torben_zapblast;
    new npc_demonic_trainer;
}
